...
export async function generateMetadata({
  ...
  const seo = websiteData?.blogPage?.seo;

  if (!seo) {
    return {
      title: "Insights and Publications",
      description: "Read our latest insights and publications",
    };
  }
...
export default async function InsightsAndPublications({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  await params; // Consume params to avoid lint warning
  const t = await getTranslations("insightsAndPublications");
  const websiteData = await getWebsiteData();

  const bannerUrl = websiteData?.blogPage?.banner
    ? `https://api-strat.othmanconstruction.com/${websiteData.blogPage.banner}`
    : "/services.webp";
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Insights Content - Client Component */}
      <InsightsContent />
    </div>
  );
}
